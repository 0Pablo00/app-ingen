...
export class ReportesPage implements OnInit {
  ...
  async cargarReporteMensual() {
    ...
    try {
      ...
      // 🔥 Obtener tareas del dueño principal y filtrar si no es admin
      const ownerUid = this.authSvc.getTasksOwnerUid();
      let tareas = await this.firebaseSvc.getFinalizedTasksByMonth(ownerUid, año, mes);
      if (this.user.role !== 'admin') {
        tareas = tareas.filter(task => task.createdBy === this.user.uid);
      }
      this.tareasDelMes = tareas;
  ...
  // ========== GENERAR PDF DE LISTA DE VERIFICACIÓN ==========
